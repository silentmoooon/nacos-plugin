...
public class ConfigInfoAggrMapperByOracle extends AbstractOracleMapper
    ...
    public MapperResult batchRemoveAggr(MapperContext context) {
        ...
        String sql =
                "DELETE FROM config_info_aggr WHERE data_id = ? AND group_id = ? AND tenant_id = NVL(?, '" + NamespaceUtil.getNamespaceDefaultId() +
                        "') AND datum_id IN ("
                        + placeholderString + ")";
    ...
    public MapperResult aggrConfigInfoCount(MapperContext context) {
        ...
        StringBuilder sql = new StringBuilder(
                "SELECT count(*) FROM config_info_aggr WHERE data_id = ? AND group_id = ? AND tenant_id = NVL(?, '" +
                        NamespaceUtil.getNamespaceDefaultId() + "') AND datum_id");
        if (isIn) {
            sql.append(" IN (");
        } else {
            sql.append(" NOT IN (");
    ...
    public MapperResult findConfigInfoAggrIsOrdered(MapperContext context) {
        ...
        String sql = "SELECT data_id,group_id,tenant_id,datum_id,app_name,content FROM "
                + "config_info_aggr WHERE data_id = ? AND group_id = ? AND tenant_id = NVL(?, '" + NamespaceUtil.getNamespaceDefaultId() +
                "') ORDER BY datum_id";
